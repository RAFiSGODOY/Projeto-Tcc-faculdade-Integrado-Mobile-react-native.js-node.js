    ...
    const fetchUserData = async () => {
        try {
            ...
            if (token) {
                ...
                setUser(userData.data_nascimento);
            }
        } catch (error) {
...
const onSubmit = async () => {
    setIsLoading(true);
    if(validarDataNascimento(newUser)){
      const dataConvertida = moment(newUser, 'DD/MM/YYYY').format("YYYY/MM/DD");
        try {
            ...
            showAndHideSuccess('Data de Nascimento Alterada!');
            setIsLoading(false);
        } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Animatable.View animation="fadeInUp" delay={200} style={styles.containerform}>
        ...
        <Feather name={'calendar'} size={22} color="#005C58" style={styles.PinEmail}/>
        <TextInputMask
          type={'datetime'}
          options={{
            format: 'DD/MM/YYYY',
          ...
          placeholder={formatarDataNascimento(user)}
        ...
        <Text style={styles.text2}>Sua Data de Nascimento, atualize caso tenha cadastrado errado.</Text>
